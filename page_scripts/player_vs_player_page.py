...
def player_vs_player_page(seasons, start_season, favourite_team):
    buli_player_season = st.sidebar.selectbox("Select Season", seasons, index=start_season)
    buli_player_season_df, _, final_players = season_player_data(season=buli_player_season)
    ...
    st.subheader(f"Player vs Player Statistics: Season {buli_player_season}")
