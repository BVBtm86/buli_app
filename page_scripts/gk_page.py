...
def gk_page(seasons, start_season, favourite_team):
    ...
    if gk_stats == "Season Stats":
        # ##### Select Season
        buli_gk_season = st.sidebar.selectbox("Select Season", seasons, index=start_season)
        buli_gk_season_df, final_total_gk, final_avg_gk = season_gk_data(season=buli_gk_season)
        ...
        st.subheader(f"Top 10 GK Statistics: Season {buli_gk_season}")
        ...
        with gk_day_col:
            ...
            gk_favourite_team = buli_gk_season_df[buli_gk_season_df['Team'] == favourite_team]['Name'].values[0]
            pos_gk = final_avg_gk.index(gk_favourite_team)
            gk_name_day = st.selectbox("Select Match Day Gk", final_avg_gk, pos_gk)
        ...
        with gk_rel_filter_col:
            ...
            gk_name = st.selectbox("Highlight Player", final_avg_gk, pos_gk)
        ...
        with gk_filter_col:
            # ##### Select Season
            buli_gk_season_df, final_avg_gk = buli_gk_data(season=seasons[-1])
