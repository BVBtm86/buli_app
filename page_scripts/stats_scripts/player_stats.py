...
def season_player_data(season):
    # ##### Read Data
    buli_df_players = pd.read_csv(f"./data/Seasons_data/Bundesliga_Players_Statistics_{season}.csv",
                                  index_col='Unnamed: 0')
    ...
    filter_players_avg = pd.DataFrame(buli_df_players.groupby('Name')['Minutes'].sum().reset_index())
...
def player_top_statistics(data, season_filter, avg_players, stat_top10, type_top10):
    ...
    top10_avg_df = data[(data[season_filter] == 1) & (data['Name'].isin(avg_players))].reset_index(drop=True)
...
def player_df_stat_season(data, team, total_players, avg_players, stat_name, stat_type, season_filter):
    ...
    team_player_avg_df = data[(data['Team'] == team) & (data['Name'].isin(avg_players))].reset_index(drop=True)
    full_player_df = data[data['Name'].isin(total_players)].reset_index(drop=True)
    full_player_avg_df = data[data['Name'].isin(avg_players)].reset_index(drop=True)
...
def player_match_day_team(data, team, players):
    ...
    season_players = list(season_player_team[season_player_team['Name'].isin(players)]['Name'].unique())
    season_players.sort()
...
def player_season_filter_stats(data, team, player, avg_players, stat_name, vs_player_type):
    ...
    team_df = data[(data['Team'] == team) & (data['Name'].isin(avg_players))].reset_index(drop=True)
...
def player_relationship_data(data, filter_type, team, player, stat_x, stat_y):
    # ##### Filter Data
    filter_df_team = data[(data['Team'] == team) & (data[filter_type] == 1)].reset_index(drop=True)
    filter_df_season = data[(data[filter_type] == 1)].reset_index(drop=True)
